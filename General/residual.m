function [worst, grid_error] = residual(Q, Q_new, grid, free_stream)
...
temp = max(q1_norm, q2_norm);
temp = max(temp, q3_norm);
grid_error = max(temp, q4_norm);

worst = max(max([q1_norm q2_norm q3_norm q4_norm]));
